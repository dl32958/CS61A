...
def pick(paragraphs, select, k):
    ...
    if k >= len(lst):
        return ''
    return lst[k]
    # lst = [p for p in paragraphs if select(p)]
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def m(paragraphs):
        lst = split(lower(remove_punctuation(paragraphs)))
        if True in [i in lst for i in topic]:
            return True
        else:
            return False
    return m
...
def accuracy(typed, source):
    ...
    if len(a) == 0 and len(b) == 0:
        ...
        score = sum([1 for i in range(len(a)) if i < len(b) and a[i] == b[i]])/len(a)
        return score * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff_words = [diff_function(typed_word, i, limit) for i in word_list]   # is a list of numbers
        min_diff = min(diff_words)
        if min_diff > limit:
            return typed_word
        else:
            return word_list[diff_words.index(min_diff)]
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0:  # Fill in the condition
        ...
        return 1 + min(add, remove, substitute)    #minimum number
...
def report_progress(typed, prompt, user_id, upload):
    ...
    # upload message
    p = {'id': user_id, 'progress': progress}
    upload(p)
    # return progress
    return progress
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in times_per_player:
        for x in range(len(i)-1):
            diff.append(i[x+1] - i[x])
        diff_lst.append(diff)
        diff = []        # should restart again
    return match(words, diff_lst)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    res_lst = [[] for i in player_indices]   #build a list [[],[]]

    for word_index in word_indices:
        time_lst = [time(match, i, word_index) for i in player_indices]  #同一个word的每个player的time的list
        a = time_lst.index(min(time_lst))    #确定最短的时间是在res_lst的哪个list中
        b = get_word(match, word_index)      #提取这个word
        res_lst[a].append(b)                 #把word append到对应的res_lst下面的list里面
    return res_lst
