...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            #print(place.ant.can_contain)
            #print(self.can_contain)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        helper = self.place
        while helper.is_hive is False:
            if not helper.bees:
                helper = helper.entrance
            else:
                return random_bee(helper.bees)
        """
        """
        step_count = 0
        helper = self.place
        while not helper.is_hive:
            if step_count < self.lower_bound or step_count > self.upper_bound:
                return None
            elif not helper.bees:
                helper = helper.entrance
                step_count += 1
            else:
                return random_bee(helper.bees)
        ...
        helper = self.place
        while not helper.is_hive:
            if len(helper.bees) and self.lower_bound <= step_count <= self.upper_bound:
                return random_bee(helper.bees)
            helper = helper.entrance
            step_count += 1            
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        for bee in self.place.bees[:]:      # lst[:] is not lst and list(lst) is not lst
            if self.health <= amount:       # if the ant's health has dropped to or below 0
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew != 0:
            ...
            self.turns_to_chew = self.chewing_turns    #Make sure to set the turns_to_chew when a Bee is eaten!
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while helper:
            if helper.ant:
                if helper.ant.double_time == 0:
                    helper.ant.double()
                    helper.ant.double_time += 1
                if helper.ant.is_container and helper.ant.ant_contained and helper.ant.ant_contained.double_time == 0:
                    helper.ant.ant_contained.double()
                    helper.ant.ant_contained.double_time += 1
            helper = helper.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # applies a temporary effect on the action method of a Bee instance
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        def bee_function(gamestate):
            if self.slow_time == 0:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not helper.is_hive:
            if helper.bees:
                for bee in helper.bees:
                    insect_dictionary[bee] = distance
            elif helper.ant and helper.ant is not self:
                if not helper.ant.is_container:
                    insect_dictionary[helper.ant] = distance
                else:
                    if helper.ant.ant_contained and helper.ant.ant_contained is not self:
                        insect_dictionary[helper.ant.ant_contained] = distance
            helper = helper.entrance
            distance += 1

        return insect_dictionary
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        self.damage =  LaserAnt.damage - 0.25 * distance - 0.0625 * self.insects_shot
        if self.damage < 0:
            self.damage = 0
        return self.damage
        # END Problem Optional 2
