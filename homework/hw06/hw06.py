...
def two_list(vals, counts):
    ...
    def build(m):
        if not m:                     # use if not to check if an iterable is empty
            return Link.empty
        else:
            return Link(m[0], build(m[1:]))
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            result = VirFib(self.prev + self.value)  #prev is a new attribute, You can create new instance attributes for objects at any point, even outside the __init__ method.
...
def is_bst(t):
    ...
    # case1: a leaf node is a BST
    if t.is_leaf():
        return True
    # case2: number of childre of node > 2
    elif len(t.branches) > 2:
        return False
    # case3: only one children node
    elif len(t.branches) == 1:
        child_branch = t.branches[0]
    # assume this is a BST structure
        return is_bst(child_branch) and (bst_max(child_branch) <= t.label or bst_min(child_branch) > t.label)
    # case4: two children nodes
    else:
        left_branch = t.branches[0]
        right_branch = t.branches[1]
        return is_bst(left_branch) and is_bst(right_branch) and left_branch <= t.label and right_branch > t.label
